...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/19 13:15:19 by escarrie          #+#    #+#             */
/*   Updated: 2021/03/19 14:15:09 by escarrie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		create_file(char *name, int i, int j)
{
	...
	while (path && name[i])
		if (name[i++] == '/')
			path--;
	ft_strlcat(bmpname, "images/", ft_strlen("images/"));
	j = 7;
	while (name[i] && name[i] != '.')
		bmpname[j++] = name[i++];
	bmpname[j] = '\0';
	ft_strlcat(bmpname, ".bmp", ft_strlen(".bmp"));
...
static void		write_file(int fd, t_scene data, t_minilibx mlx)
{
	char	*pixel_array;
	int		image_size;
	int		i;
...
void			save_as_bmp(t_minilibx mlx, t_scene data, char *name)
{
	...
	create_header(data, &head_file, &dibhead);
	write_header(fd, head_file, dibhead);
